...
mdl.add_constraints(mdl.sum(gamma_e * s *(alpha * c[j, k] * (f_e[j, k, r, s] + m_e * delta_e[j, k, r, s]) + beta * c[j, k] * mdl.sum(((v[l]/60) ** 2) * g_e[j, k, r, l, s] for l in L)) for j, k in A for r in R for s in F) <= C_e + (1440 - mdl.sum((c[j, k] / v[l]) * g_e[j, k, r, l, s] * s for j, k in A for l in L) - mdl.sum(U[j]*nu[j, s]*s*sigma_e[j, r, s] for j in N)) * pi for r in R for s in F)

# time constraint for electric trucks
mdl.add_constraints(mdl.sum((c[j, k]/v[l]) * g_e[j, k, r, l, s]*s for j, k in A for l in L) + mdl.sum(U[j]*nu[j, s]*s*sigma_e[j, r, s] for j in N) <= 1440 for r in R for s in F)


# time constraint for ICE trucks
mdl.add_constraints(mdl.sum((c[j, k]/v[l]) * g_c[j, k, r, l, s]*s for j, k in A for l in L) + mdl.sum(U[j]*nu[j, s]*s*sigma_c[j, r, s] for j in N) <= 1440 for r in R for s in F)
