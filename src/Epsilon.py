...
# original function
# mdl.minimize(
#              mdl.sum(omega * s * u[r, s] for r in R for s in F) + # visit stuff
#              mdl.sum(((f_e[j, k, r, s] + m_e * delta_e[j, k, r, s]) * c[j, k]) * alpha * c_e * gamma_e * s for j, k in A for r in R for s in F) + # load induced costs (electric)
#              mdl.sum(((f_c[j, k, r, s] + m_c * delta_c[j, k, r, s]) * c[j, k]) * alpha * (c_c + e) * gamma_c * s for j, k in A for r in R for s in F) + # load induced costs (conventional)
#              mdl.sum((mdl.sum((v[l]/60) ** 2 * g_e[j, k, r, l, s] for l in L) * c[j, k] * beta) * c_e * gamma_e * s for j, k in A for r in R for s in F) + # speed induced costs (electric)
#              mdl.sum((mdl.sum((v[l]/60) ** 2 * g_c[j, k, r, l, s] for l in L) * c[j, k] * beta) * (c_c + e) * gamma_c * s for j, k in A for r in R for s in F) + # speed induced costs (conventional)
#              mdl.sum(t_e[r] for r in R) * o_e + # electric truck costs
...
# TODO: augmented epsilon constraint (truck costs + visits vs. pollution/fuel costs). Parameter range: 20000
########################################################################################################
    ...
    if solution.solve_status.value == 2:
        ...
        for r in R:
            if solution.get_value(x[r]) == 1:
                ...
                print('     Fuel/Electricity costs (total per km) [€/km]: ' + str(
                    round((fuel_costs + fuel_costs_2) / distance, 2)))
                ...
                print('     Energy consumption [kWh/km]: ' + str(
